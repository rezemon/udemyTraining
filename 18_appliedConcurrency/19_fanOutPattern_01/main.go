...
func main() {
	...
	//c := gen(2, 3, 4, 5)
	//testing what happen,
	//no matter how many times i run, output is the same ??? why
	//perhaps make more complicated formula
	c := gen(5, 4, 3, 2, 13, 10, 18, 7, 12, 11)
	...
	*/
	// now to do fanIn
	for n := range merge(cSq, c100, cSq2) {
		fmt.Println(n)
	}
...
func time100(aCh chan int) chan int {
	outSq := make(chan int)
	go func() {
		for n := range aCh {
			outSq <- n * 100 //square of n
		}
		close(outSq)
	}()
	return outSq
