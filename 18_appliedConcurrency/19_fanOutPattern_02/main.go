...
func mergeB(manyChIn ...chan int) chan int {
	out := make(chan int)
	var wg sync.WaitGroup

	//declare a anonymous function
	//start an output goRoutine for each input channel in manyChIn
	//output copies values from aCh to out  until aCh is closed, then call wg.Done
	output := func(aCh chan int) {
		for n := range aCh {
			out <- n
		}
		wg.Done()
	...
	wg.Add(len(manyChIn))
	for _, value := range manyChIn {
		go output(value)
	}

	//start a goRoutine to close out
	//once all the output goRoutine are done
	//this must start after the wg.Add
	go func() {
		wg.Wait()
		close(out)
	}()
